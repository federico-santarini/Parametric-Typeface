...
class Tools:
    ...
    def cubeQuarter(self, glyph,
        ...
        pen = glyph.getPen()
        pen.moveTo((p_x1, p_y1))
        pen.qCurveTo((cExt_x1, cExt_y1), (cExt_x2, cExt_y2), (cExt_x3, cExt_y3))
        pen.lineTo((p_x2, p_y2))
        pen.qCurveTo((cInt_x1, cInt_y1), (cInt_x2, cInt_y2), (cInt_x3, cInt_y3))
        pen.lineTo((p_x1, p_y1))
        pen.closePath()
    ...
    # Retta
    def lin (self, x,x1,y1,x2,y2):
        y=(x-x1)*(y2-y1)/(x2-x1)+y1
        return y    
    
    
    # Parabola (di Roberto Arista)
    def par (self, x,x1,y1,x2,y2,x3,y3):
    #Calcolo del sistema di equazioni lineari
       
       a = numpy.array([[x1**2,x1,1], [x2**2,x2,1], [x3**2,x3,1]])
       b = numpy.array([y1,y2,y3])
       E = numpy.linalg.solve(a, b)

    
    #Conversione da matrice a singole variabili numeriche
    ...
    #Calcolo della soluzione dell'incognita
